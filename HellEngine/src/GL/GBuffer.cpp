...
namespace HellEngine
{
	...
	void GBuffer::Configure(int width, int height)
	{
		...
		glBindTexture(GL_TEXTURE_2D, gNormal); // alpha contains DECAL BLOCKOUT
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_FLOAT, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	...
	/*	glBindTexture(GL_TEXTURE_2D, gBloodDecals);
		...
		*/
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT8, GL_TEXTURE_2D, gAlbedoDecalComposite, 0);
		(/*/
