...
namespace HellEngine
{
   ...
   //     std::cout << "BONEINFO SIZEEEEE: " << skinnedModel->m_BoneInfo.size() << "\n";
    ...
    void FileImporter::GrabSkeleton(SkinnedModel* skinnedModel, const aiNode* pNode, int parentIndex)
    {
        ...
        parentIndex = skinnedModel->m_skeleton.m_joints.size(); // don't do your head in with why this works, just be thankful it does.

        skinnedModel->m_skeleton.m_joints.push_back(joint);
