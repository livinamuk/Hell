...
#ifdef DEBUG
#define DEBUG_ANIM_CURVE_EVALUATION
#endif
...
namespace ImCurveEdit
{
   ...
   int Edit(Delegate &delegate, const ImVec2& pos, const ImVec2& size, unsigned int id, const ImRect *clippingRect, ImVector<EditPoint> *selectedPoints)
   {
      bool& selectingQuad = delegate.GetSelectionQuad();
      ImVec2& quadSelection = delegate.GetQuadSelection();
      int& overCurve = delegate.GetOverCurve();
      int& movingCurve = delegate.GetMovingCurve();
      bool& scrollingV = delegate.GetScrollingV();
      std::set<EditPoint>& selection = delegate.GetSelection();
      bool& overSelectedPoint = delegate.GetOverSelectedPoint();

      int ret = 0;

	  ImVec2 cursorOldPos = ImGui::GetCursorScreenPos();
	  ImGui::SetCursorScreenPos(cursorOldPos + pos);
      ...
      const ImVec2 offset = ImGui::GetCursorScreenPos() + ImVec2(0, size.y);
...
#ifdef DEBUG_ANIM_CURVE_EVALUATION
	  static float evalPos = 0, speed = 0.001f;

	  float const curveVal = delegate.Evaluate(evalPos);
	  ImVec2 center;
	  ImVec2 scPos = ImGui::GetCursorScreenPos();
	  center.x = evalPos * size.x + scPos.x;
	  center.y = curveVal * size.y + scPos.y;

	  draw_list->AddCircleFilled(center, 5.0f, 0xFFFF0000);
	  ...
	  ImGui::SetCursorScreenPos(cursorOldPos);
