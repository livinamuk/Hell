...
void BloodEffect::Draw(Shader* shader, HellEngine::Camera const& camera, Transform& global)
{
	if (m_CurrentTime == AnimationSeconds)
		return;
	...
	shader->use();
	shader->setMat4("u_MatrixProjection", camera.m_projectionMatrix);
	shader->setMat4("u_MatrixView", camera.m_viewMatrix);
